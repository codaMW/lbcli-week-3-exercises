# Create a 1-of-4 P2SH multisig address from the public keys in the four inputs of this raw tx:

transaction=""

#decode the transaction

decoded_transaction=$(bitcoin-cli -regtest -rpcwallet=btrustwallet decoderawtransaction $transaction)

#extract the public keys
pubKey1=$(echo $decoded_transaction | jq -r '.vin[0].txinwitness[-1]')
pubKey2=$(echo $decoded_transaction | jq -r '.vin[1].txinwitness[-1]')
pubKey3=$(echo $decoded_transaction | jq -r '.vin[2].txinwitness[-1]')
pubKey4=$(echo $decoded_transaction | jq -r '.vin[3].txinwitness[-1]')

#For debugging purposes
#echo "key1: $pubKey1"
#echo "key2: $pubKey2"
#echo "key3: $pubKey3"
#echo "key4: $pubKey4"

bitcoin-cli -regtest -rpcwallet=btrustwallet createmultisig 1 '["'$pubKey1'","'$pubKey2'","'$pubKey3'","'$pubKey4'"]' "p2sh-segwit"
